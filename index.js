...
const addRole = async () => {
    let answers = await inquirer.prompt(addRoleQuestion)
    console.log(answers);
    // .then(answer => {
    //     let addRoleQuery = `INSERT INTO role(title) VALUES ('${answer.role_name}');`
    //     connection.query(addRoleQuery, (err, result) => {
    //         if (err) console.log(err);
    //         console.log(`Role with title ${answer.role_name} added successfully.`);
    //         startQuestions();
        // });
    // });
};
